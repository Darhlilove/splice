 ...
 */
export async function storeApiKey(specId, apiKey) {
 ...
 */
export async function getApiKey(specId) {
    const redis = getRedisClient();
    const key = `${API_KEY_PREFIX}${specId}`;
    let apiKey = null;
    if (redis) {
 ...
 */
export async function deleteApiKey(specId) {
    const redis = getRedisClient();
    const key = `${API_KEY_PREFIX}${specId}`;
    if (redis) {
        await redis.del(key);
 ...
 */
export async function validateApiKey(specId, providedKey) {
 ...
 */
export async function hasApiKey(specId) {
    const redis = getRedisClient();
    const key = `${API_KEY_PREFIX}${specId}`;
    if (redis) {
