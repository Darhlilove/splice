...
describe("MockServerManager Error Parsing", () => {
    it("should parse EMISSINGPOINTER errors correctly", () => {
        ...
        expect(mockPrismError).toContain('token "components" in "#/components/schemas/Pet" does not exist');
    });
    it("should identify missing schema references", () => {
        const errorMessage = 'at "#/endpoints/2/responses/200/content/application~1xml/schema/items", token "components" in "#/components/schemas/Pet" does not exist';
        const refMatch = errorMessage.match(/token "([^"]+)" in "([^"]+)" does not exist/);
