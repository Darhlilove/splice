...
export class MockServerManager {
    constructor() {
        ...
        this.PORT_RANGE_START = 4010;
        this.PORT_RANGE_END = 4099;
        this.MAX_PORT_RETRIES = 10;
        this.prismInstalled = null;
    }
    /**
     ...
     */
    getPrismInstallationInstructions() {
        return ("Prism CLI is not installed. Please install it using one of the following methods:\n\n" +
     ...
     */
    async requiresApiKeyAuth(specId) {
        try {
            ...
            return Object.values(securitySchemes).some((scheme) => scheme.type === "apiKey" || scheme.type === "http");
        }
        catch (error) {
     ...
     */
    async startServer(specId, config) {
        // Check if Prism is installed
        if (!this.isPrismInstalled()) {
            throw new Error(`Prism CLI is not installed. ${this.getPrismInstallationInstructions()}`);
        }
        ...
        try {
            ...
            if (config.port) {
                ...
                if (!isAvailable) {
                    console.warn(`Requested port ${config.port} is not available, finding alternative...`);
            ...
            while (attempts < this.MAX_PORT_RETRIES) {
                try {
                    const process = await this.spawnPrismProcess(specId, config.specPath, port, host);
                    ...
                    if (lastError.message.includes("EADDRINUSE") ||
                        ...
                        console.warn(`Port ${port} conflict detected (attempt ${attempts + 1}/${this.MAX_PORT_RETRIES}), trying next port...`);
                        attempts++;
            ...
            throw new Error(`Failed to start mock server after ${this.MAX_PORT_RETRIES} port conflict retries. ` +
    ...
    async stopServer(specId) {
        ...
        return new Promise((resolve, reject) => {
            ...
            process.once("exit", async () => {
                ...
                this.servers.set(specId, Object.assign(Object.assign({}, serverInfo), { status: "stopped" }));
                resolve();
            });
    ...
    async findAvailablePort(startPort) {
        ...
        throw new Error(`Could not find available port after ${this.MAX_PORT_RETRIES} attempts`);
    }
    /**
     ...
     */
    parseSpecError(errorOutput) {
        try {
            // Check for EMISSINGPOINTER error code (common pattern)
            if (errorOutput.includes("EMISSINGPOINTER") ||
                ...
                const refMatch = errorOutput.match(/token "([^"]+)" in "([^"]+)" does not exist/);
                if (refMatch) {
            ...
            if (errorOutput.includes("{") && errorOutput.includes("}")) {
                ...
                if (jsonMatch) {
                    try {
                        ...
                        if (errorObj.code === "EMISSINGPOINTER" ||
                            errorObj.name === "MissingPointerError") {
                            const message = errorObj.message || "";
                            // Extract the missing reference
                            const refMatch = message.match(/token "([^"]+)" in "([^"]+)" does not exist/);
                            if (refMatch) {
                                const [, token, reference] = refMatch;
     ...
     */
    async spawnPrismProcess(specId, specPath, port, host) {
        return new Promise((resolve, reject) => {
            var _a, _b;
            ...
            (_b = process.stderr) === null || _b === void 0 ? void 0 : _b.on("data", (data) => {
                ...
                if (!hasStarted) {
                    ...
                    if (output.includes("EADDRINUSE") ||
                        ...
                        reject(new Error(`Port ${port} is already in use (EADDRINUSE). Will retry with next available port.`));
                    }
                    else if (output.includes("ResolverError") ||
            ...
            process.on("exit", (code, signal) => {
                if (!hasStarted) {
                    reject(new Error(`Prism exited immediately with code ${code} and signal ${signal}. ` +
    ...
    monitorProcess(specId, process) {
        ...
        process.on("exit", (code, signal) => {
            ...
            console.log(`[Prism ${specId}] Process exited with code ${code} and signal ${signal} after ${uptimeSeconds}s`);
            const serverInfo = this.servers.get(specId);
            if (serverInfo && serverInfo.status === "running") {
                ...
                this.servers.set(specId, Object.assign(Object.assign({}, serverInfo), { status: "stopped", error: crashReason }));
            }
        ...
        process.on("error", (error) => {
            ...
            const serverInfo = this.servers.get(specId);
            if (serverInfo) {
                this.servers.set(specId, Object.assign(Object.assign({}, serverInfo), { status: "error", error: `Process error: ${error.message}` }));
            }
        });
        ...
        process.on("disconnect", () => {
            ...
            const serverInfo = this.servers.get(specId);
            if (serverInfo && serverInfo.status === "running") {
                this.servers.set(specId, Object.assign(Object.assign({}, serverInfo), { status: "stopped", error: "Process disconnected unexpectedly" }));
            }
        });
    }
    /**
    ...
    async cleanup() {
        ...
        for (const specId of this.processes.keys()) {
            stopPromises.push(this.stopServer(specId).catch((error) => {
