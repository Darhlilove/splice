...
export class SDKGenerator {
   ...
   */
  private async checkOpenAPIGeneratorAvailable(): Promise<boolean> {
   ...
   */
  private getOpenAPIGeneratorInstallInstructions(): string {
...
For more information, visit:
  ...
  async generateSDK(
    ...
    console.log(
      `  Spec Info: ${spec.info?.title || "Unknown"} v${spec.info?.version || "Unknown"
      }`
    );
    ...
    if (!isAvailable) {
      ...
      this.logGenerationMetrics(
        ...
        startTime,
        false,
        "OpenAPI Generator not available"
      );
      return {
        success: false,
        error: errorMessage,
    ...
    if (!validationResult.valid) {
      ...
      console.error(
        `  Errors: ${JSON.stringify(validationResult.errors, null, 2)}`
      );
      this.logGenerationMetrics(
        generationId,
        startTime,
    ...
    console.log(
      `[SDKGenerator] Generation state initialized for ${generationId}`
    );
    ...
    try {
      ...
      const outputPath = await this.executeOpenAPIGenerator(
        spec,
        config,
        generationId,
        (progress) => {
      ...
      console.log(
        `[SDKGenerator] Generation ${generationId} completed successfully`
      );
      ...
      if (error instanceof Error) {
        ...
        console.error(
          `[SDKGenerator] Unknown error during generation ${generationId}`
        );
      ...
      setTimeout(() => {
        console.log(
          `[SDKGenerator] Cleaning up generation state for ${generationId}`
        );
  ...
  private async validateSpec(spec: OpenAPISpec): Promise<void> {
    ...
    if (!spec.openapi && !spec.swagger) {
      validationErrors.push(
        "Missing OpenAPI version field (openapi or swagger)"
      );
    }
    ...
    if (spec.paths) {
      for (const [path, pathItem] of Object.entries(spec.paths)) {
        ...
        const hasOperation = operations.some(
          (op) => pathItem[op as keyof typeof pathItem]
        );
   ...
   * @throws Error if validation fails
   */
  private async validateSpecWithGenerator(spec: OpenAPISpec): Promise<void> {
    ...
    try {
      ...
      const { stdout, stderr } = await execAsync(
        `npx openapi-generator-cli validate -i ${tempSpecPath}`
      );
      ...
      throw new Error(
        `OpenAPI specification validation failed. Please ensure your spec is valid according to the OpenAPI specification.\n\nError: ${error instanceof Error ? error.message : "Unknown error"
        }`
      );
    }
  }
   ...
   */
  private formatSpecValidationErrors(errors: string[]): string {
...
Please fix these issues and try again.`;
  ...
  private async executeOpenAPIGenerator(
    ...
    return new Promise((resolve, reject) => {
      ...
      const timeout = setTimeout(() => {
        ...
        reject(
          new Error(
            `SDK generation timed out after ${this.GENERATION_TIMEOUT_MS / 1000
            } seconds. Try simplifying your OpenAPI specification or breaking it into smaller parts.`
          )
        );
      ...
      try {
        ...
        processChild.stdout?.on("data", (data) => {
          ...
          if (output.includes("writing file")) {
            if (onProgress) {
              onProgress(
                Math.min(90, (stdout.match(/writing file/g) || []).length * 5)
              );
            }
          }
        });
        ...
        processChild.on("close", (code) => {
          ...
          if (code === 0) {
            ...
            const unsupportedFeatures = this.parseUnsupportedFeatures(
              stdout,
              stderr
            );
            ...
            if (unsupportedFeatures.length > 0) {
              reject(
                new Error(
                  this.formatUnsupportedFeaturesError(unsupportedFeatures)
                )
              );
            } else {
        ...
        reject(
          new Error(
            `Failed to prepare SDK generation: ${error instanceof Error ? error.message : "Unknown error"
            }`
          )
        );
      }
   ...
   */
  private parseUnsupportedFeatures(stdout: string, stderr: string): string[] {
   ...
   */
  private formatUnsupportedFeaturesError(features: string[]): string {
...
If you believe this is an error, please check the OpenAPI Generator logs for more details.`;
   ...
   */
  private async generateAndWriteReadme(
    spec: OpenAPISpec,
    config: SDKConfig,
    outputPath: string
  ): Promise<void> {
   ...
   */
  private async packageAsZip(outputPath: string): Promise<string> {
    return new Promise((resolve, reject) => {
      ...
      output.on("close", () => {
        ...
        console.log(
          `[SDKGenerator] ZIP archive created: ${zipPath} (${sizeInMB} MB)`
        );
        resolve(zipPath);
      });
